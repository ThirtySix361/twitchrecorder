                        ...
                        <div class="recordingOptions">
                            <div class="recordingOptionsHead">
                                <?php if ( $_GET["channel"] != "" && $_GET["action"] != "stopRecordingTask" ) { echo "<div style='width: 100%; text-align: center;'>".$_GET["channel"]."</div>";?>
                                    <button onclick="location.href='?settings&channel=<?= $_GET['channel'] ?>&action=tasklog'"> log task </button>
                                    <button onclick="location.href='?settings&channel=<?= $_GET['channel'] ?>&action=ffmpeglog'"> log ffmpeg </button>
                            ...
                            <div class="recordingOptionsContent"><?php
                                    if ( $_GET["action"] != "" && $_GET["channel"] != "" ) {
                                        ...
                                        if ( $action == "tasklog" ) { echo ($log = file_get_contents($taskLogPath)) !== false ? $log : 'log empty'; }
                                        if ( $action == "ffmpeglog" ) { echo ($log = file_get_contents($ffmpegLogPath)) !== false ? $log : 'log empty'; }
                                        if ( $action == "logfix" ) { echo ($log = file_get_contents($fixLogPath)) !== false ? $log : 'log empty'; }
                                        if ( $action == "logthumbnail" ) { echo ($log = file_get_contents($thumbnailLogPath)) !== false ? $log : 'log empty'; }
                                    }
                            ?></div>
                        </div>
                    </div>
