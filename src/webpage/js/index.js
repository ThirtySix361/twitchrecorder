...
function toggleMultiSelect(init = false) {
    var multiselect = localStorage.getItem("settings_multiSelect");
    if (multiselect === null) { multiselect = false; init = false; } else { multiselect = multiselect === "true" }
    var icon = document.querySelector("#toggleMultiSelect i");
    var text = document.querySelector("#toggleMultiSelect span");
    if (init) { multiselect = !multiselect; } else { localStorage.setItem("settings_multiSelect", !multiselect); }
    if (multiselect) {
        icon.className = "fa-solid fa-check";
        text.innerHTML = "single select"
    } else {
        icon.className = "fa-solid fa-check-double";
        text.innerHTML = "multi select"
    }
}
toggleMultiSelect(true)

function toggleVideolistening(init = false) {
    var grouped = localStorage.getItem("settings_groupToggle") === "true";
    var icon = document.querySelector("#groupToggle i");
    var text = document.querySelector("#groupToggle span");
    if (init) { grouped = !grouped; } else { localStorage.setItem("settings_groupToggle", !grouped); getVideos(); }
    if (grouped) {
        icon.className = "fa-solid fa-bars-staggered";
        text.innerHTML = "grouped"
    } else {
        icon.className = "fa-solid fa-bars";
        text.innerHTML = "ungrouped"
    }
}
toggleVideolistening(true)

function toggleDescription(init = false) {
    var grouped = localStorage.getItem("settings_descriptionToggle") === "true";
    var icon = document.querySelector("#descriptionToggle i");
    var text = document.querySelector("#descriptionToggle span");
    if (init) { grouped = !grouped; } else { localStorage.setItem("settings_descriptionToggle", !grouped); }
    if (grouped) {
        document.documentElement.style.setProperty('--hint-display', 'inline');
        icon.className = "fa-solid fa-lightbulb";
        text.innerHTML = "show hints"
    } else {
        document.documentElement.style.setProperty('--hint-display', 'none');
        icon.className = "fa-regular fa-lightbulb";
        text.innerHTML = "hide hints"
    }
}
toggleDescription(true)

function notifyModal(msg) {
    var string = `<div id="notifyModal"><i class="fa-solid fa-circle-info"></i><span>${msg}<span></div>`;
    renderModal(string);
}
...
async function deleteSeen(filename, skipconfirm = false) {
    if (!skipconfirm) { if (!(await renderModal(getModalOptionsString("Do you really want to reset this recording?", { "yes": true, "no": false }), { "yes": true, "no": false }))) { return; } }
    ...
    return `
        <div class="video" tabindex="0" id="${video.filename}" channel="${video.name}">
            ...
            <div class="videosettings">
                <a href="?getVideo=${video.path}" onclick="event.preventDefault(); videoModal('${video.url_video}','${video.url_log}', '${video.filename}', '${video.path}')"><i class="fa-solid fa-play"></i></a>
                ...
                <div onclick="(async () => { await renderModal(getModalOptionsString('Do you really want to delete this recording permanently?', {'yes':true, 'no':false}), {'yes':true, 'no':false}) && deleteVideo('#${video.filename}', '${video.path}') })()"><i class="fa-solid fa-trash"></i></div>
            </div>
        </div>
    `;
}

function renderModal(content, options = false, evalafter = false) {
    var overlay = document.createElement("div");
    overlay.className = "modaloverlay";

    var modal = document.createElement("div");
    modal.className = "modalbox";
    modal.innerHTML = "<div class='modalcontent'>" + content + "</div>";
    ...
    if (options && typeof options === 'object') {
        Object.keys(options).forEach(function (key) {
            var el = modal.querySelector('[modalbtn="' + key + '"]');
...
function getModalOptionsString(msg, options) {
    var html = `
        <div id="notifyOptionsModal">
            <div class="notifyOptionsIcon"><i class="fa-solid fa-question"></i></div>
            <div class="notifyOptionsTitle">` + msg + `</div>
            <div class="notifyOptionsButtons">
    `;

    Object.keys(options).forEach(function (key) {
        html += `<div class="notifyOptionsButton" modalbtn="` + key + `">` + key + `</div>`;
    });
...
async function getChannels() {
    ...
    window.toggleChannel = function (el, channel) {
        var multiselect = localStorage.getItem("settings_multiSelect") === "true";
...
async function getVideo(filepath) {
    ...
    var result = await apiFetch(url);
    if (!result) { return; }
    videoModal(result.url_video, result.url_log, result.filename, result.path)
}
...
async function addModal() {
    ...
    renderModal(string, false, evalafter);
...
async function videoModal(videourl, chaturl, filename, filepath) {
    var targetPath = `?getVideo=${filepath}`;
    ...
    var string = `<div id="videoModal">
        <video id="video" src="${videourl}" filename="${filename}" controls></video>
        <div id="chat" chaturl="${chaturl}"></div>
    </div>`;
    ...
    function updateChatHeight() {
        if (window.innerWidth < 800) {
            chat.style.height = (window.innerHeight - video.clientHeight - 50) + 'px';
        } else {
            chat.style.height = video.clientHeight + 'px';
        }
    }

    video.addEventListener('loadedmetadata', updateChatHeight);
